...
void ROS2Carla::EnablePX4Bridge(void* vehicle, std::string ros_name) {
  if (_px4_bridge) {
    DisablePX4Bridge();
  }
...
void ROS2Carla::DisablePX4Bridge() {
  if (_px4_bridge) {
    _px4_bridge->Destroy();
    _px4_bridge.reset();
...
bool ROS2Carla::IsPX4BridgeEnabled() {
  return _px4_bridge && _px4_bridge->IsAlive();
}

void ROS2Carla::SetPX4VehicleControlCallback(PX4ControlCallback callback) {
  if (_px4_bridge && _px4_bridge->IsAlive()) {
    _px4_bridge->SetVehicleControlCallback(std::move(callback));
  } else {
    log_warning("SetPX4VehicleControlCallback called before PX4 bridge is enabled");
  }
}

void ROS2Carla::UpdatePX4Bridge(
  ...
  // HasNewActuatorControls fires the VehicleControlCallback inline if registered
